// frontend/src/Pages/student-poll/StudentPollPage.jsx
...
import { socket } from "../../socket"; // <- shared socket (note relative path)
...
const StudentPollPage = () => {
  ...
  // ----------------------------
  // 1. Attach kickedOut listener immediately
  // ----------------------------
  useEffect(() => {
    const handleKickedOut = (data) => {
      // optional: show toast/alert quickly
  ...
  // ----------------------------
  // 2. Fetch active poll for late joiners (ChatPopover will have emitted joinChat)
  // ----------------------------
  useEffect(() => {
    socket.emit("getActivePoll", (pollData) => {
      if (pollData) {
  ...
  useEffect(() => {
    const handlePollCreated = (pollData) => {
      ...
      setTimeLeft(Math.floor((pollData.endTime - Date.now()) / 1000));
      setPollId(pollData._id);
    };

    const handlePollResults = (updatedVotes) => setVotes(updatedVotes);

    const handlePollEnded = ({ pollId: endedPollId }) => {
